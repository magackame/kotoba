module Api.User.Posts exposing (Request, Response(..), request)

import Api
import Api.Post as Post
import Api.User.Posts.Preferences as Preferences exposing (Preferences)
import Http
...
responseInnerDecoder tag =
    case tag of
        ...
        _ ->
            D.fail <| "Encountered unknown tag `" ++ tag ++ "` while decoding User.Posts.Response"
